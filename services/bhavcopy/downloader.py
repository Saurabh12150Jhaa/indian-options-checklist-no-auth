    ...
    for d in days:
        ...
        try:
            ...
            if resp.status_code == 200 and len(resp.content) > 500:
                dest = BHAVCOPY_CACHE_DIR / new_format_filename(d)
...
def _download_nse_old_format(
    ...
    for d in days:
        ...
        try:
            resp = session.get(url, headers=_DOWNLOAD_HEADERS, timeout=20)
            if resp.status_code == 200 and len(resp.content) > 500:
                dest = BHAVCOPY_CACHE_DIR / old_format_filename(d)
                dest.write_bytes(resp.content)
                touch_access(dest)
                downloaded += 1
                if progress_cb:
...
def download_bhavcopies(
    ...
    result.already_had = sum(1 for d in days if already_have(d))
    ...
    for source_name, source_fn in _SOURCES:
        ...
        still_missing = sum(1 for d in days if not already_have(d))
        if still_missing == 0:
    ...
    # Final tally
    final_missing = sum(1 for d in days if not already_have(d))
    if final_missing > 0:
        result.messages.append(
